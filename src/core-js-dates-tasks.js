...
function getNextFriday(date) {
  ...
  const daysTo = dayFrom < 5 ? 5 - dayFrom : 7 - (dayFrom - 5);
  startDate.setDate(startDate.getDate() + daysTo);
  return startDate;
}
...
function getCountDaysInMonth(month, year) {
  const givenDate = new Date(year, month, 0);
...
function isDateInPeriod(date, period) {
  ...
  const seconds = dateToCheck.getTime();
  const dateStart = new Date(period.start);
  const secondsStart = dateStart.getTime();
  const dateEnd = new Date(period.end);
  const secondsEnd = dateEnd.getTime();
  return (
    (seconds > secondsStart && seconds < secondsEnd) ||
